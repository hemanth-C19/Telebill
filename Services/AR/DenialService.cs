...
public class DenialService(IArRepository repo, INotificationService notificationService) : IDenialService
{
        ...
        int denialId, UpdateDenialStatusDto dto)
    {
        ...
        if (string.Equals(target, "Resolved", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(target, "WrittenOff", StringComparison.OrdinalIgnoreCase))
        {
            var claimId = denial.ClaimId ?? 0;
            var arUsers = await repo.GetUsersByRoleAsync("AR");
            foreach (var user in arUsers)
            {
                var message = string.Equals(target, "Resolved", StringComparison.OrdinalIgnoreCase)
                    ? $"Denial #{denial.DenialId} for Claim #{claimId} has been resolved"
                    : $"Denial #{denial.DenialId} for Claim #{claimId} has been written off. Amount: ${denial.AmountDenied ?? 0m}";
        ...
        if (string.Equals(status, "Open", StringComparison.OrdinalIgnoreCase))
        {
            ...
            foreach (var user in arUsers)
            {
                await notificationService.CreateAsync(new CreateNotificationDto
                {
                    UserId = user.UserId,
                    Category = "Denial",
                    Message = $"Appeal document uploaded for Claim #{denial.ClaimId ?? 0}. Denial status advanced to Appealed"
                });
            }
        }
        ...
        ResetClaimForResubmissionDto dto)
    {
        ...
        var arUsersForReset = await repo.GetUsersByRoleAsync("AR");
        foreach (var user in arUsersForReset)
        {
            await notificationService.CreateAsync(new CreateNotificationDto
            {
                UserId = user.UserId,
                Category = "Denial",
                Message = $"Claim #{denial.ClaimId.Value} has been reset to Draft for resubmission"
            });
        }
